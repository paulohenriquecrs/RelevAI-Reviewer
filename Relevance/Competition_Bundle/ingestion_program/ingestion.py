...
class Ingestion:
    ...
    def transfrom_data(self):
        ...
        self.df["most_relevant_dict"] = self.df["most_relevant"].apply(
            self._text_to_dict
        )
        self.df["second_most_relevant_dict"] = self.df["second_most_relevant"].apply(
            self._text_to_dict
        )
        self.df["second_least_relevant_dict"] = self.df["second_least_relevant"].apply(
            self._text_to_dict
        )
        self.df["least_relevant_dict"] = self.df["least_relevant"].apply(
            self._text_to_dict
        )

        # Convert from dictionary to text
        self.df["most_relevant_text"] = self.df["most_relevant_dict"].apply(
            self._dict_to_paragraphs
        )
        self.df["second_most_relevant_text"] = self.df[
            "second_most_relevant_dict"
        ].apply(self._dict_to_paragraphs)
        self.df["second_least_relevant_text"] = self.df[
            "second_least_relevant_dict"
        ].apply(self._dict_to_paragraphs)
        self.df["least_relevant_text"] = self.df["least_relevant_dict"].apply(
            self._dict_to_paragraphs
        )
    ...
    def prepare_data(self):
        ...
        X["text"] = (
            self.df["most_relevant_text"].tolist()
            + self.df["second_most_relevant_text"].tolist()
            + self.df["second_least_relevant_text"].tolist()
            + self.df["least_relevant_text"].tolist()
        )
        y = (
            self.df["most_relevant_label"].tolist()
            + self.df["second_most_relevant_label"].tolist()
            + self.df["second_least_relevant_label"].tolist()
            + self.df["least_relevant_label"].tolist()
        )
